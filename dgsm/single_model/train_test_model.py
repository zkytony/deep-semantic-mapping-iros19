...
KNOWN_CLASSES = ['corridor', 'door', 'medium_room']
...
def parse_args():
    ...
    learn_params.add_argument('--threshold', type=str, default='-500,-500,-500',
                              help='Threshold for semi-supervised data')
    ...
    #     sys.exit(1)
    if args.subset not in [1, 2, 3, 4, 5, 6, 7, 8, 9]:
...
def print_args(args):
    ...
    print("* Semi-supervised threshold: %s" % args.threshold)
...
def main():
    ...
    data = Data(args.angle_cells, args.radius_min,
                args.radius_max, args.radius_factor, KNOWN_CLASSES)
    data.load(args.data_dir)
    data.process(args.subset, args.occupancy_vals)
    data.print_info()
    # data.load_masked_scans(args.results_dir)
    ...
    # Model
    model = PlaceModel(data=data, place_classes=KNOWN_CLASSES,
    ...
    model.semisup_train(stop_condition=args.stop_condition,
                        additive_smoothing_min=args.smoothing_min,
                        additive_smoothing_decay=args.smoothing_decay,
                        results_dir=args.results_dir,
                        threshold=[float(th) for th in args.threshold[1:-1].split(',')])
