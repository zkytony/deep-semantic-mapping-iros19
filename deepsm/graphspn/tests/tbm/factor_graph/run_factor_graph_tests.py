...
from spn_topo.tbm.topo_map import Edge
from spn_topo.tbm.template import SingleEdgeTemplate, PairEdgeTemplate, ThreeNodeTemplate, PairTemplate
from spn_topo.tests.tbm.runner import TbmExperiment, doorway_policy, random_policy, random_fixed_policy, random_fixed_plus_placeholders, get_noisification_level
...
class FactorGraphTest:
    ...
    def _visualize_test_case(self, seq_id, topo_map, groundtruth=None, masked=None, result=None, consider_placeholders=False):
        """Visualize"""
    ...
    def run_instance(self, seq_id, topo_map, masked, groundtruth, likelihoods=None,
        ...
        if result_path is None:
            result_path = os.path.join(self._result_dir, "%s_%s_Inf_%d.json" % (self._db_test, seq_id, COUNT))
            COUNT += 1

        # Run the binary for BP
        bp_proc = subprocess.Popen([self._bp_exec_path, fg_path, tab_path, self._infer_type, result_path])
        bp_proc.wait()

        if self._infer_type == "marginal":
            res_catg_map = self._parse_marginal_results(result_path, varlabels)  # (Using marginal inference)
        else:
            res_catg_map = self._parse_mpe_results(result_path, varlabels)  # (Using MPE inference)
        stats = self.compute_accuracy(groundtruth, masked, res_catg_map, used_likelihoods=likelihoods is not None,
                                      consider_placeholders=consider_placeholders, topo_map=topo_map)
    ...
    def compute_accuracy(self, groundtruth, masked, result, used_likelihoods=False, consider_placeholders=False, topo_map=None):
        """
...
def run_test_suite(mask_policy, dataset, amount=-1, tiny_dataset=None, policy_args={},
    ...
    for db in dbs:
        ...
        ft = FactorGraphTest(BP_EXEC_PATH, dataset, list(set(dbs) - {db}),  db, result_dir=BP_RESULT_DIR,
