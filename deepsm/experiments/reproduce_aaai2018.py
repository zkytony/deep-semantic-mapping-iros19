...
from deepsm.util import CategoryManager, print_banner, print_in_box, ColdDatabaseManager, json_safe
...
def test_inference_noisy(sess,
    ...
    for seq_id in topo_maps:
        _test_single_graph(sess, testdb, seq_id, topo_maps[seq_id],
                           template_spns, nrounds,
                           hic, loc, hiic, loic,
...
def _test_single_graph(sess, db_name, seq_id, topo_map, template_spns, nrounds,
    ...
    viz_partitions_dirpath = os.path.join("results", "aaai18", "noisy", "partitions-%s-%s" % (db_name, seq_id))
    
    instance_spn = NodeTemplateInstanceSpn(topo_map, sess,
                                           *[(tspn , tspn.template) for tspn in template_spns],
                                           num_partitions=num_partitions,
                                           visualize_partitions_dirpath=viz_partitions_dirpath,
                                           seq_id=seq_id,
                                           db_name=db_name,
                                           extra_partition_multiplyer=extra_partition_multiplyer)
    ...
    for rr in range(nrounds):
        ...
        print("Inferring most likely categories")
        start = time.time()
        ...
        if save_results:
            save_path = os.path.join("results", "aaai18", "noisy", db_name + "-" + seq_id + "-round_" + str(rr))
        ...
        if load_if_exists and os.path.exists(tspn_path):
            ...
            if template == SingletonTemplate:
                ...
                samples_by_db = dataset.create_template_dataset(template,
                                                                ...
                                                                db_names=traindbs)
                samples = []
                for db in samples_by_db:
                    samples.extend(samples_by_db[db])
